...
def _get_collision_list(result_list: list, obstacles_speed: float):
    ...
    for result_obj in result_list:
        if result_obj.obstacle_velocity == obstacles_speed:
            lst.append(get_avg_model_collisions(result_obj.capf_results))
    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_collision_per_parameter += get_avg_model_collisions(result_obj.capf_results)
                counter += 1
...
def _get_avg_step_per_paramater(obstacles_speed = None):
    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_steps_per_parameter += get_avg_model_time_steps(result_obj.capf_results)
                counter += 1
            elif obstacles_speed == result_obj.obstacle_velocity:
                avg_steps_per_parameter += get_avg_model_time_steps(result_obj.capf_results)
                counter += 1
...
def _get_avg_path_per_paramater(obstacles_speed = None):
    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_path_lengt_per_parameter += get_avg_path_length(result_obj.capf_results)
                counter += 1
            elif obstacles_speed == result_obj.obstacle_velocity:
                avg_path_lengt_per_parameter += get_avg_path_length(result_obj.capf_results)
                counter += 1
...
def show_avg_collisions_per_paramater(per_parameter=False):
    if not per_parameter:
        ...
        plt.xticks([r for r in range(len(bars1))], ['0.5', '1', '3', '5', '10', '20'])
...
def show_avg_func_per_parameter(func, per_parameter=False, y_label=''):
    ...
    if not per_parameter:
        ...
        plt.bar(r3, bars3, width=barWidth, edgecolor='white', label='1.5')
        plt.bar(r4, bars4, width=barWidth, edgecolor='white', label='2')
        # plt.bar(r1, bars1, color='tab:blue', width=barWidth, edgecolor='white', label='0.5')
        # plt.bar(r2, bars2, color='tab:orange', width=barWidth, edgecolor='white', label='1')
        # plt.bar(r3, bars3, color='tab:green', width=barWidth, edgecolor='white', label='1.5')
        # plt.bar(r4, bars4, color='tab:red', width=barWidth, edgecolor='white', label='2')
...
def show_avg_func_per_parameter_for_min_dist(func, per_parameter=False, y_label=''):
    if not per_parameter:
        barWidth = 0.3
        ...
        # Add xticks on the middle of the group bars
        plt.xlabel(f'Different force distances', fontweight='bold')
        plt.ylabel(y_label, fontweight='bold')
        plt.xticks([r for r in range(len(bars1))], ['1', '3', '5', '10'])
...
def analyze_different_rep_forces():
    ...
    # show_avg_collisions_per_paramater(per_parameter=False)
    # show_avg_collisions_per_paramater(per_parameter=True)
    # show_collision_bars_selection(obstacles_speed=0.5)
    # show_collision_bars_selection(obstacles_speed=1)
    # show_collision_bars_selection(obstacles_speed=2)
    # show_collision_bars_all(obstacles_speed=0.5)
    # show_collision_bars_all(obstacles_speed=1)
    # show_collision_bars_all(obstacles_speed=2)


def analyze_different_min_dist():
    show_avg_func_per_parameter_for_min_dist(func=_get_avg_collisions_per_paramater, per_parameter=False, y_label='Avg. collisions')
    show_avg_func_per_parameter_for_min_dist(func=_get_avg_collisions_per_paramater, per_parameter=True, y_label='Avg. collisions')

    show_avg_func_per_parameter_for_min_dist(func=_get_avg_step_per_paramater, per_parameter=False, y_label='Avg. calculation steps')
    show_avg_func_per_parameter_for_min_dist(func=_get_avg_step_per_paramater, per_parameter=True, y_label='Avg. calculation steps')

    show_avg_func_per_parameter_for_min_dist(func=_get_avg_path_per_paramater, per_parameter=False, y_label='Avg. path length')
    show_avg_func_per_parameter_for_min_dist(func=_get_avg_path_per_paramater, per_parameter=True, y_label='Avg. path length')


if __name__== '__main__':
    
    if len(sys.argv) <= 1:
        BASE_PATH = 'Result_Objects/classic_apf_results/'
        ADDITIONAL_PATH = 'different_rep_forces/'
        # ADDITIONAL_PATH = 'different_min_dist/'
    else:
        try:
    ...
    analyze_different_rep_forces()
    # analyze_different_min_dist()

    # print(_get_avg_collisions_per_paramater())
