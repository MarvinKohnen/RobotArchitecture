    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_collision_per_parameter += get_avg_model_collisions(result_obj.iapf_results)
                counter += 1
...
def _get_avg_step_per_paramater(obstacles_speed = None):
    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_steps_per_parameter += get_avg_model_time_steps(result_obj.iapf_results)
                counter += 1
            elif obstacles_speed == result_obj.obstacle_velocity:
                avg_steps_per_parameter += get_avg_model_time_steps(result_obj.iapf_results)
                counter += 1
...
def _get_avg_path_per_paramater(obstacles_speed = None):
    ...
    for test in results_objects_list:
        ...
        for result_obj in test:
            if obstacles_speed is None:
                avg_path_lengt_per_parameter += get_avg_path_length(result_obj.iapf_results)
                counter += 1
            elif obstacles_speed == result_obj.obstacle_velocity:
                avg_path_lengt_per_parameter += get_avg_path_length(result_obj.iapf_results)
                counter += 1
...
def show_avg_func_per_parameter(func, per_parameter=False, y_label=''):
    ...
    if not per_parameter:
        ...
        plt.xlabel(f'Different scaling terms', fontweight='bold')
        plt.ylabel(y_label, fontweight='bold')
        # plt.xticks([r for r in range(len(bars1))], ['0', '1', '5', '10', '20', '50'])
        plt.xticks([r for r in range(len(bars1))], ['0', '0.1', '1', '5', '10', '20', '50'])
...
if __name__== '__main__':
    ...
    if len(sys.argv) <= 1:
        BASE_PATH = 'Result_Objects/iapf_results/'
        ADDITIONAL_PATH = 'different_vel_forces/'
    else:
        try:
