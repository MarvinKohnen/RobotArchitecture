...
SCALING_FACTOR_REPULSIVE_ROTATION_FORCE = SCALING_FACTOR_REPULSIVE_FORCE*3
...
class Updater:
    ...
    def _get_controll_commands_1(self, current_point, current_orientation):
        ...
        # non_relativ_force_vektor = potential_force(current_point, current_orientation)
        non_relativ_force_vektor = potential_force(current_point, current_point)
...
def movement_force2(point: np.array, obstacle: Velocity_Circle, current_orientation_robot: np.array):
    ...
    if movement[0] == 0 and movement[1] == 0:# or obstacle.get_shell_distance(point) > np.linalg.norm(movement * REPULSIVE_FORCE_FUTURE_COUNT) + MIN_DISTANCE_REPULSIVE_FORCE:  # um Performanc zusparen ggf
        ...
        if distance < MIN_DISTANCE_REPULSIVE_FORCE:
            ...
            rep_force_with_rotation = get_rotational_repulsive_force(temp, current_orientation_robot, obstacle.get_position() - point, obstacle.get_position() + movement * REPULSIVE_FORCE_FUTURE_COUNT - point)
...
def get_rotational_repulsive_force(temp_term: np.array, current_direction: np.array, obstacle_position: np.array, obstacle_future_position: np.array):
    ortho_direction_right = np.dot(CLOCKWISE_MATRIX, current_direction)
...
def draw_rotational_forward_example():
    ...
    show_map(with_path=False, max_vector_norm=4)
    # show_map(with_path=True, path_steps=300, max_vector_norm=5)

    test_map_multiple_paths(path_count=10, max_vector_norm=5, path_steps=500)
